<?php
// src/Zeldanet/PinterestBundle/Command/UpdateBrokenCommand.php
...
class UpdateBrokenCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
                ->setName('pinterest:updatebroken')
                ->setDescription('UpdateBroken pins')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // Get broken PINs URLs
        $pins = array();
        $query = $em->createQuery(
                'SELECT p FROM ZeldanetPinterestBundle:Pins p WHERE p.image=\'\' '
        )
            ->setMaxResults(100)
        ;
        ...
        $output->writeln('Pinterest bundle updated broken: ' . $total . ' pins~ ');
    }
